...
**Chapter 18** breaks the assumption that tokens are produced one at a time. Chapter 1
...
**Chapter 19** breaks the assumption that the model may produce any token. When output must
...
**Chapter 20** breaks the assumption that a batch is homogeneous. Some requests want
