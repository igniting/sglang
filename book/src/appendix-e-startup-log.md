...
Chapter 3 covers the topology; `_launch_scheduler_processes`
...
multi-node deployment to stall silently, which is why the watchdogs of Chapter 5 exist.
...
`python/sglang/srt/model_executor/model_runner.py:1057` `load_model` (Chapter 12) brackets
...
Slow loading is usually I/O; Chapter 12's `set_num_threads(1)` and load-format notes are the
...
`python/sglang/srt/mem_cache/memory_pool.py:1668` `_finalize_allocation_log`, from Chapter 9.

**`#tokens` is the single most important number in the log.** It is
`max_total_num_tokens` — the budget Chapter 6 spends, shared across all concurrent requests.
...
larger than expected (check Phase 4), a KV dtype that did not take effect (Chapter 15), or
KV head replication under high TP (Chapter 13).
...
(Chapter 14). Worth reading even when you did not set the flag, because the *resolved*
...
Chapter 15. Three things to read here.
...
**The batch-size list** is what Chapter 15's bucketing captured. A batch outside this list
...
`--cuda-graph-max-bs`. Note the ordering — Chapter 7 explained that graph capture runs
...
case Chapter 9 quoted — before real traffic arrives. It is also the first end-to-end proof
...
At this point `/health_generate` (Chapter 3) will run a real forward pass, which is a
...
`python/sglang/srt/observability/startup_time.py` (Chapter 22), prints the per-phase
breakdown.
...
Chapter 22's tuning order is an adjustment to one of them.
...
| --- | --- | --- |
| Hang with no error | 3 | Distributed init: address, ports, world size (Ch. 16) |
| OOM during capture | 7 | `--mem-fraction-static` too high (Ch. 9, 14) |
| `#tokens` far lower than expected | 5 | Larger weights, KV dtype not applied, or KV head replication (Ch. 13, 14) |
...
| Second request unexpectedly slow | 8 | Warmup did not cover a JIT path (Ch. 9) |
