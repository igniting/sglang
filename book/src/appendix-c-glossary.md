...
regardless of batch shape. Required for deterministic inference. *Ch. 7*

**Chunked prefill** — splitting a long prompt across scheduler iterations so it does not
stall active decodes. Trades the long request's TTFT for everyone else's ITL. *Ch. 5*

**Continuous batching** — admitting and retiring requests every step rather than running a
batch to completion. Really an admission-control problem under a memory budget. *Ch. 5*

**CP (context parallelism)** — splitting a sequence across ranks. *DCP* is the
decode-time variant that splits the KV cache within a TP group. *Ch. 15*

**CUDA graph** — a recorded sequence of GPU operations replayed with one launch, removing
per-kernel CPU overhead. Requires static shapes and stable buffer addresses. *Ch. 14*
...
exactly one continuation. *Ch. 19*
...
(temporal, height, width) positions. *Ch. 20*
...
cost of pipeline bubbles. *Ch. 15*
...
constrained decoding and tool parsing. *Ch. 19*
